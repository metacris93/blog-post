    ...
    <article class="article" v-for="(article, index) in articles" :key="index">
      ...
      <div class="article-details">
        ...
        <ul>
          ...
          <li>Publicación: {{ getPublishedAt(article.publishedAt) }}</li>
        </ul>
        <div class="article-link">
          <b-button variant="primary" :href="article.url"
            >Ver Contenido</b-button
          >
        </div>
      </div>
    </article>
...
export default {
  ...
  methods: {
    ...
    getPublishedAt(publishedAt) {
      if (publishedAt) {
        return moment(publishedAt).locale("es").calendar();
      }
      return "";
    },
    getInitialArticles() {
